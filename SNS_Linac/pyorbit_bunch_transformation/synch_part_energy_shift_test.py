...
def printBunch(bunch):
	...
	print "synch part Ekin [MeV] =  %16.9g "%(eKin*1000.)
	print "synch part time [us]  =  %16.9g "%(bunch.getSyncParticle().time()*1.0e+6)
	for ind in range(n_parts):
		(x,xp,y,yp,z,dE) = (bunch.x(ind),bunch.xp(ind),bunch.y(ind),bunch.yp(ind),bunch.z(ind),bunch.dE(ind))
		print "i= %3d       (z,dE) [mm,MeV] = ( %16.9g , %16.9g )"%(ind,z*1000.,(eKin+dE)*1000.)
	print "     --- distances ---"
	for ind in range(n_parts-1):
		(x,xp,y,yp,z,dE) = (bunch.x(ind),bunch.xp(ind),bunch.y(ind),bunch.yp(ind),bunch.z(ind),bunch.dE(ind))
		(x1,xp1,y1,yp1,z1,dE1) = (bunch.x(ind+1),bunch.xp(ind+1),bunch.y(ind+1),bunch.yp(ind+1),bunch.z(ind+1),bunch.dE(ind+1))
		print "i= %3d  delta(z,dE) [mm,MeV] = ( %16.9g , %16.9g )"%(ind,(z-z1)*1000.,(dE - dE1)*1000.)
...
rfGap_node.setLength(0.)
rfGap_node.setParam("E0TL",E0TL)
rfGap_node.setParam("E0L",E0TL)
rfGap_node.setParam("mode",0.)
rfGap_node.setParam("gap_phase",gap_phase*math.pi/180.)
...
cav.setPhase(gap_phase*math.pi/180.)
...
for delta_E in (0.,0.00001):
	#delta_E = 0.000
	bunch_tmp = Bunch()
	bunch.copyBunchTo(bunch_tmp)
	print "======================================================== CASE"
	print "delta_E [MeV] = ",delta_E*1000.
...
for delta_Z in (0.,0.001):
	bunch_tmp = Bunch()
	bunch.copyBunchTo(bunch_tmp)
	print "======================================================== CASE"
	print "delta_Z [mm] = ",delta_Z*1000.
	redefCalc = SynchPartRedefinitionZdE()
