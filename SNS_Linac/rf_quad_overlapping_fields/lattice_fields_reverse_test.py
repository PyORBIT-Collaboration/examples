...
def getQuadFiledsDistribution(seq_name,xml_file_name,rf_field_location, \
	...
	accLattice = sns_linac_factory.getLinacAccLattice([seq_name,],xml_file_name)
	...
	if(use_overlapped_fields):
		...
		Replace_BaseRF_Gap_and_Quads_to_Overlapping_Nodes(accLattice,z_step,rf_field_location,[seq_name,],[],SNS_EngeFunctionFactory)
...
for seq_name in names:
	...
	(z_arr,g_arr,gp_arr,ez_arr) = getQuadFiledsDistribution(seq_name,xml_file_name,rf_field_location,\
		                                                use_overlapped_fields = True, reverse_latt = False)
	
	(z1_arr,g1_arr,gp1_arr,ez1_arr) = getQuadFiledsDistribution(seq_name,xml_file_name,rf_field_location,\
		                                                use_overlapped_fields = True, reverse_latt = True)
