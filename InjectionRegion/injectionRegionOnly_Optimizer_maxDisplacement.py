...
from KevinPython.maxDisplacementNode import MaxDisplacement_Node
import argparse
...
class MyScorer(Scorer):
	...
	def __init__(self,xTarget,pxTarget,yTarget,pyTarget,zTarget,dETarget,maxXDisplacementTarget):
		...
		self.maxXDisplacementTarget=maxXDisplacementTarget
	...
	def getScore(self,trialPoint):
		...
		#fileOut.close()
		myMDNode=MaxDisplacement_Node("myMDNode")
		...
		#i = 0
		for node in nodes:
			node.addChildNode(myMDNode,AccNode.EXIT)
		#	pass
		#	if node.getName().strip() == "DH_A12":
		...
		score = (b.x(0)-self.xTarget)**2 + (b.px(0)-self.pxTarget)**2 + (b.y(0)-self.yTarget)**2+(b.py(0)-self.pyTarget)**2+(b.z(0)-self.zTarget)**2+(b.pz(0)-self.dETarget)**2+(paramsDict["maxXDisplacement"]-self.maxXDisplacementTarget)**2
		#print "paramsDict[\"maxXDisplacement\"]=", paramsDict["maxXDisplacement"]
		return score	
...
scorer = MyScorer(0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,-0.0939824727476)
