...
# change the sign of all quads' fields	
for quad in quads:
...
#----------------------------------------------------------------------
# After this point you can change the ETL0 parameters for the caities.
#----------------------------------------------------------------------

etl0_change_param = ["CCL4",0.0]

rf_cavs = accLattice.getRF_Cavities()
for rf_cav in rf_cavs:
	rf_gaps = cav.getRF_GapNodes()
	coeff = 1.0
	if(rf_cav.getName().find(etl0_change_param[0]) >= 0):
		coeff = etl0_change_param[1]
	else:
		continue
	for rf_gap in rf_gaps:
		...
		if(math.sqrt(xMax**2+yMax**2) > r_max):
			...
			for ind in range(nParticles):
				...
				if(math.sqrt(x**2+y**2) > r_max):
					# write down lost-particles' coordinates
					eKin = bunch.getSyncParticle().kinEnergy()*1.0e+3
					loss= "%7.5f   %7.5f    %7.5f    %7.5f  %7.5f  %7.5f" %(pos,x,px,y,py,eKin)
					print "loss", loss
					fl_loss_pos_coord_out.write(loss + "\n")
					#
